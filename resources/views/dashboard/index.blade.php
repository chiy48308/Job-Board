    ...
    {{--    Profile Info Form--}}
        ...
        <div class="bg-white p-8 rounded-lg shadow-md w-full">
            ...
            <form method="POST" action="{{route("profile.update")}}" enctype="multipart/form-data">
                ...
                <button type="submit" class="w-full bg-green-500 text-white px-4 py-2 border rounded focs:outline-none">Save</button>
            </form>
        </div>
    {{--    Job Listing--}}
    <div class="bg-white p-8 rounded-lg shadow-md w-full">
        ...
        @forelse($jobs as $job)
            <div class="flex justify-between items-center border-b-2 border-gray-200 py-2">
                <div>
                    <h3 class="text font-semibold">{{$job->title}}</h3>
                    <p class="text-gray-700">{{$job->job_type}}</p>
                </div>
                <div class="flex space-x-3">
                    <a href="{{route('jobs.edit', $job->id)}}" class="bg-blue-500 text-white px-4 py-2 rounded text-sm">Edit</a>
                    <!-- Delete Form -->
                    <form method="POST" action="{{route('jobs.destroy', $job->id)}}?from=dashboard" onsubmit="return confirm('Are you suer that you want to delete this job?')">
                        @csrf
                        @method('DELETE')
                        <button
                            type="submit"
